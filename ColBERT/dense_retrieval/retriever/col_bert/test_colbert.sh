...
python -m dense_retrieval.retriever.col_bert.colbert.data_prep \
    --dataset ${dataset} \
    --split ${test_split} \
...
# # ########################################################################### 
CUDA_VISIBLE_DEVICES=${cudaNum} python -m  dense_retrieval.retriever.col_bert.colbert.index \
    --root $OUTPUT_DIR \
    --doc_maxlen 350 \
    --mask-punctuation \
    --bsize 512 \
...
CUDA_VISIBLE_DEVICES=${cudaNum} python -m dense_retrieval.retriever.col_bert.colbert.index_faiss \
    --index_root $INDEX_ROOT \
    --index_name $INDEX_NAME \
    ...
    --root $OUTPUT_DIR \
    --experiment ${dataset}
echo "Faiss Index done"

# ####################################################################################
# # 3. Retrieval: retrieve relevant documents of queries from faiss index checkpoint #
# ####################################################################################

CUDA_VISIBLE_DEVICES=${cudaNum} OMP_NUM_THREADS=30 python -m dense_retrieval.retriever.col_bert.colbert.retrieve \
    --amp \
    --doc_maxlen 350 \
    --mask-punctuation \
    --bsize 32 \
...
# ######################################################################
python -m dense_retrieval.retriever.col_bert.colbert.beir_eval \
    --dataset ${dataset} \
    --split ${test_split} \
    --data_dir ${raw_data_dir}\
    --log_dir ${LOG_DIR} \
